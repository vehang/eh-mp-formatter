...
const ACCESS_TOKEN_EXPIRE_TIME = 7 * 24 * 60 * 60 * 1000; // 7 天（毫秒）
const ACCESS_TOKEN_PREFIX = 'ac_';

// 内存中的 accessToken 映射: Map<accessToken, { barCode, createdAt, expiresAt }>
const appAccessTokens = new Map();

/**
 * 加载 accessToken 数据（启动时调用）
 */
function loadAppTokens() {
    try {
        ...
        valid.forEach(t => {
            appAccessTokens.set(t.accessToken, {
                barCode: t.barCode,
...
/**
 * 持久化 accessToken 数据到文件
 */
function saveAppTokens() {
    try {
        const data = [];
        appAccessTokens.forEach((info, token) => {
 ...
 * 生成 accessToken
 */
function generateAccessToken(barCode) {
    const token = ACCESS_TOKEN_PREFIX + crypto.randomBytes(32).toString('hex');
    const now = Date.now();
    const info = {
        barCode,
        createdAt: now,
        expiresAt: now + ACCESS_TOKEN_EXPIRE_TIME
    };
    // 同一 barCode 旧 token 失效
    for (const [key, val] of appAccessTokens.entries()) {
        if (val.barCode === barCode) {
            appAccessTokens.delete(key);
        }
    }
    appAccessTokens.set(token, info);
    saveAppTokens();
...
function isValidAccessToken(token) {
    const info = appAccessTokens.get(token);
    if (!info) return false;
    if (info.expiresAt <= Date.now()) {
        appAccessTokens.delete(token);
        saveAppTokens();
        return false;
    }
    return true;
...
function getAppTokenInfo(token) {
    return appAccessTokens.get(token) || null;
}

/**
 * 读取 appAuthRequired 配置
 */
function isAppAuthRequired() {
    try {
        const content = fs.readFileSync(VERSION_FILE, 'utf8');
        const versionInfo = JSON.parse(content);
        return versionInfo.appAuthRequired === true;
    } catch (error) {
        return false; // 默认不开启
    }
}
...
/**
 * App API 认证中间件
 * 从多种来源提取 accessToken 并校验
 */
function appAuthMiddleware(req, res, next) {
    // 检查是否开启了认证
    if (!isAppAuthRequired()) {
        return next();
    }
...
/**
 * App 认证接口：用 token + barCode 换取 accessToken
 * POST /api/app/auth
 * 
 * 不受 appAuthMiddleware 保护
 */
app.post('/api/app/auth', async (req, res) => {
    try {
        ...
        console.log('[App认证] 开始验证:', { token: token.substring(0, 10) + '...', barCode });
        ...
        console.log('[App认证] 验证结果:', result);

        // 验证成功
        if (result.code === 0 || result.success) {
            const tokenInfo = generateAccessToken(barCode.trim());

            console.log('[App认证] 验证成功，已生成 accessToken, barCode:', barCode.trim());
        ...
        // 验证失败
        console.log('[App认证] 验证失败:', result.msg || result.message);
        ...
        console.error('[App认证] 请求失败:', error);
        return res.json({
            code: 1000,
 ...
 */
app.post('/api/config/simple', appAuthMiddleware, async (req, res) => {
    try {
 ...
 * 返回之前通过 POST /api/config/simple 保存的参数缓存
 */
app.get('/api/config/simple', appAuthMiddleware, (req, res) => {
    try {
 ...
 * 无需登录验证
 */
app.post('/api/fix', appAuthMiddleware, async (req, res) => {
 ...
 * 自动停止已有任务，启动新任务
 */
app.post('/api/weixin/qr/start', appAuthMiddleware, async (req, res) => {
    try {
 ...
 * 返回当前任务的状态和二维码链接
 */
app.get('/api/weixin/qr/status', appAuthMiddleware, (req, res) => {
    try {
        // 检查状态文件是否存在
