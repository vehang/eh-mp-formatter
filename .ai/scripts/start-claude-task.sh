...
rm -f /home/node/.openclaw/workspace/.ai/scripts/*.lock
...
if tmux has-session -t claude-code 2>/dev/null; then
    ...
    if ! tmux capture-pane -t claude-code -p 2>/dev/null | grep -q "Claude Code"; then
        ...
        tmux send-keys -t claude-code 'cd /home/node/.openclaw/workspace && /home/node/.openclaw/npm-global/bin/claude --dangerously-skip-permissions' Enter
        sleep 5
    fi
else
