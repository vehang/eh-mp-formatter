...
1. 复制模板文件：
   ```bash
   cp backup-configs/openclaw.json.template /home/node/.openclaw/openclaw.json
   ```
...
### 恢复 Skills
```bash
cp -r backup-configs/github-tools-publisher /home/node/.openclaw/skills/
cp -r backup-configs/multi-agent-dev /home/node/.openclaw/skills/
cp -r backup-configs/wechat-ai-publisher /home/node/.openclaw/skills/
```

### 恢复 Cron 任务
```bash
cp backup-configs/cron-jobs.json /home/node/.openclaw/cron/jobs.json
```

## 不备份的内容（敏感/运行时数据）

- `/home/node/.openclaw/credentials/` - 凭证文件
- `/home/node/.openclaw/sessions/` - 会话数据
- `/home/node/.openclaw/logs/` - 日志文件
- `/home/node/.openclaw/delivery-queue/` - 消息队列
